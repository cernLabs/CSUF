...
for (b in 2:(B + 1)) {
sig2n.p <- (kappa0 * (mu.post.gibbs[(b - 1)] - mu0)ˆ2 + nu0 * sig20 + (n - 1) * sˆ2 + (n *(ybar - mu.post.gibbs[(b - 1)])ˆ2))/nu_n.p
sig2.post.gibbs[(b)] <- rinvchisq(1, df = nun.p,
scale = sig2n.p) #sig2| mu,data
