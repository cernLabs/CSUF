...
Use importance sampling with standardized weights to estimate $\theta=Var(X)$, where $X$ is a random variable with a density proportional to $f(x)=\sin(\sin(x))$ with support $0 \leq x \leq \pi/2$. Use 10,000 random values form the standard normal distribution (the proposal density) using the following specific R code:
...
Use stamnardized weights for your resampling. Resample 5000 values, and use the resampled values to estimate $\theta=Var(X)$. Include your R code and print the estimate of $Var(X)$ with at least 6 decimal places.
...
> find the conditionals to be exponential distributions
