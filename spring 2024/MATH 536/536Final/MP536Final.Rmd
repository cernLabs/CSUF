...
```{r}
TenFold <- function(ntree.in, mfinal.in, cp.in, ms.in){
...
CV.ind.mat = matrix(sample(1:n,3070,replace=F),nrow=10)
...
for(k in 1:10){
 ...
 model.a = randomForest(equa1, data = train.data, ntree = 500, mtry = 3, control=rpart.control(minsplit=ms.in,cp=cp.in))
 model.b = boosting(equa1, data = train.data, mfinal=100, control=rpart.control(minsplit=ms.in,cp=cp.in))
...
for(i.tr in c(100,500,1000)){
  for(i.cp in c(.001,.01,.05)){
    for(i.ms in c(5,10,20)){
      row <- TenFold(i.tr,i.tr,i.cp,i.ms)
      LLdf <- rbind(LLdf,row)
    }
  }
}
